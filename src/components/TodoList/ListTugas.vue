<template>
    <v-main class="list">
        ...
        <v-card v-if="checkedItem.length > 0" class="my-4">
            <v-card-text>
                <p style="text-align: left">Delete Multiple:</p>
                <ul v-for="(item, index) in checkedItem" :key="index" style="text-align:left">
                    <li>{{ item }}</li>
                </ul>
                <v-layout left>
                    <v-btn color="error" style="margin-top: 25px" @click="multiDelete">Hapus Semua</v-btn>
                </v-layout>
            </v-card-text>
        </v-card>
    ...
    methods: {
        ...
        multiDelete() {
            for(var i = 0; i < this.checkedItem.length; i++){
                this.todos.splice(this.todos.findIndex(e => e.task === this.checkedItem[i]),1);
            }
            this.checkedItem= [];
        },
    },
